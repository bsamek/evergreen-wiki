...
.. list-table:: **Parameters**
   ...
   * - task_name
     - string
     - Optional. Only return tasks with the corresponding name.
...
.. list-table:: **ProjectVars**
   ...
   * - vars
     - map[string]string
     - Map of the variable to its value (if private, value is redacted)
   * - private_vars
     - map[string]bool
     - Indicates whether that variable should be private. (NOTE: once a variable has been set to private this cannot be undone)
   * - vars_to_delete
     - []string
...
Returns a paginated list of all projects. Note aliases, variables, and subscriptions may not be populated.
...
Copy Redacted Variables to an Existing Project
`````````````````````````````````````````````

::

 POST /projects/<source_project>/copy_redacted_variables?dest_project=<new_project_id>&dry_run=<boolean>

Restricted to admins of the source project and the destination project.
...
Grants the user specified by user_id the permissions in the request body. The format of the body is:
::
