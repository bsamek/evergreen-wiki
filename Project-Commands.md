...
logically nonsensical.
  ...
  ------------------- -------- -------------------------------------------
  `status`            string   The final status of the test. Should be one
                      (enum)   of: \"fail\", \"pass\", \"silentfail\",
                               \"skip\".

  `test_file`         string   The name of the test. This is what will be
                               displayed in the test results section of
                               the UI as the test identifier.

  `group_id`          string   The group ID if the test is associated with
                               a group. This is mostly used for tests
                               logging directly to cedar.

  `url`               string   The URL containing the rich-text view of
                               the test logs.

  `url_raw`           string   The URL containing the plain-text view of
                               the test logs.

  `line_num`          int      The line number of the test within the
                               \"url\" parameter, if the URL actually
                               contains the logs for multiple tests.

  `exit_code`         int      The status with which the test command
                               exited. For the most part this does
                               nothing.

  `task_id`           string   The ID of the task with which this test
                               should be associated. The test will appear
                               on the page for the specified task.

  `execution`         int      The execution of the task above with which
                               this test should be associated.
  ------------------------------------------------------------------------
...
-   <https://github.com/evergreen-ci/shrub>
-   <https://github.com/evergreen-ci/shrub.py>
-   <https://github.com/evergreen-ci/shrub.ts>
...
-   generate.tasks cannot redefine functions, tasks, or variants.
    generate.tasks may only append new functions, tasks, and variants.
    It is a validation error to modify an existing static or dynamic
    function, task, or variant, or to define one twice in the JSON
    document passed to the command. It is legal to specify a variant
    multiple times in order to add additional tasks to the variant, as
    long as no other variant metadata is changed.
...
-   `revisions`: For commit builds, each module should be passed as
    ...
    means that for patch builds, editing the \"modules\" section of the
    project config will not change the checked out hash.
...
-   `branch`: must be the name of branch, commit hashes [are not
    accepted]{.title-ref}
...
-   `ipv6`- Set to true if instance should have [only]{.title-ref} an
...
what is set as the `remote_file` field. The filter uses the same
...
This command runs a shell script. The environment variable \"CI\" will
be set to \"true\" in scripts originating from this command.
...
-   `env`: a map of environment variables and their values. Unless
    overridden, the environment variable \"CI\" will be set to \"true\"
    and \"GOCACHE\" will be set to a subdirectory within the working
    directory.
...
-   `background`: if set to true, does not wait for the script to exit
...
-   `continue_on_err`: if set to true, causes command to exit with
    success regardless of the script's exit code
...
you can also run a [#!]{.title-ref} script as if it were a binary. To
get similar behavior on Windows, try [bash.exe -c
yourScript.sh]{.title-ref}.
...
-   `continue_on_err`: if true, do not mark task as failed if command
    fails
-   `add_expansions_to_env`: when true, add all expansions to the
...
The `subprocess.scripting` command runs a command inside of a scripting
environment/harness like a Python Virtual Environment or a GOPATH. It
wraps [jasper\'s scripting
harness](https://godoc.org/github.com/mongodb/jasper/scripting) and
mirrors the `subprocess.exec` interface and behavior as closely as
possible.
...
- command: subprocess.scripting
  params:
    ...
    packages:
      - "pymongo"
      - "pyyaml"
    lock_file: ${workdir}/src/requirements.txt
    command: "buildscripts/smoke.py --continue_on_error"
    harness_path: ".venv"
```

Parameters:

-   `harness` : specify the type of scripting harness to use. Use
    `python` (defaults to python3), `python2`, `golang`, or `roswell`
    (common-lisp).
-   `command`: the command line args, as a string to run in the
    environment. You can specify a command either as an argument or as a
    command but not both.
-   `args`: an argv list of strings to run as a command in the
    environment. You can specify a command either as an argument or as a
    command but not both.
-   `test_dir`: the directory where tests should be run. This cannot be
    used if command or args is set.
-   `test_options`: the options that describe how the tests will execute
    in the `test_dir`. Behavior of these options depends on which
    harness is used. Options within `test_options`:
...
-   `cache_duration_secs`: the duration to cache the configuration.
    Defaults to 10 minutes. Once the duration has elapsed, Evergreen
    will reconfigure the environment.
-   `cleanup_harness`: By default, harnesses are not removed when the
    command terminates, which means that multiple commands can reuse the
    same environment, if they\'re identically configured. When this is
    true, Evergreen cleans up the harness before the command executes.
-   `lock_file`: Specifies dependencies in a lock file (e.g.
    `requirements.txt`), if the harness supports it.
-   `packages`: A list of packages that will be installed in the
    environment.
-   `harness_path`: A path, within the working directory where the
    harness will be located.

Additional, execution parameters, common with `subprocess.exec`:
...
-   `continue_on_err`: if true, do not mark task as failed if command
    fails
-   `add_expansions_to_env`: if true, add all expansions to the
    command\'s environment.
-   `include_expansions_in_env`: specify one or more expansions to
    include in the environment. If you specify an expansion that does
    not exist, it is ignored.
